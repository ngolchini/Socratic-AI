...
class ClinicalCaseTutor:
    ...
    def __init__(self):
        ...
        if not env_path.exists():
            self.logger.error(f"No .env file found at {env_path.absolute()}")
            st.error("No .env file found. Please ensure .env file exists in the project root.")
            st.stop()
            
        load_dotenv(dotenv_path=env_path, override=True)
        # Initialize OpenAI client and LLM manager
        api_key = os.getenv("OPENAI_API_KEY")
        api_key = "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"
        if not api_key:
